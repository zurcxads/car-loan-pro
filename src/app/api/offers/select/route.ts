  ...
  try {
    ...
    if (isSupabaseConfigured()) {
      ...
      const { error: selectError } = await supabase
        ...
        .eq('id', offer.id);
      if (selectError) {
        return apiError('Failed to update selected offer', 500);
      }
      ...
      const { error: declineError } = await supabase
        ...
        .neq('status', 'declined');
      if (declineError) {
        return apiError('Failed to update sibling offers', 500);
      }
      ...
      if (linkError) {
        return apiError('Failed to link application to lender', 500);
      }
    } else {
