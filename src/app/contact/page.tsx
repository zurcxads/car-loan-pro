    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="min-h-screen bg-white ">
      <section className="pt-28 pb-16 px-6">
        <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-2xl mx-auto">
          ...
          <motion.p variants={fadeUp} className="mt-4 text-lg text-gray-600  text-center font-light">
            Have a question? We&apos;re here to help.
          </motion.p>

          <motion.div variants={fadeUp} className="mt-12 grid sm:grid-cols-3 gap-6 mb-12">
            <div className="rounded-2xl border border-gray-200  bg-gray-50  p-6 text-center hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
          ...
          {submitted ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl border border-blue-200  bg-blue-50  p-10 text-center">
            ...
            <motion.form variants={fadeUp} onSubmit={handleSubmit} className="rounded-2xl border border-gray-200  bg-white  shadow-sm p-8 space-y-5">
              ...
              <div>
                <label htmlFor="contact-name" className="block text-xs text-gray-600  mb-2 font-medium">Name</label>
                <input
                  id="contact-name"
                  type="text"
                  value={name}
                  ...
                  required
                  className="w-full px-4 py-3 bg-gray-50  border border-gray-200  rounded-xl text-sm text-gray-900  placeholder-gray-400  focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 transition-colors duration-200"
                  placeholder="Your name"
                />
              </div>
              <div>
                <label htmlFor="contact-email" className="block text-xs text-gray-600  mb-2 font-medium">Email</label>
                <input
                  id="contact-email"
                  type="email"
                  value={email}
              ...
              <div>
                <label htmlFor="contact-subject" className="block text-xs text-gray-600  mb-2 font-medium">Subject</label>
                <select
                  id="contact-subject"
                  value={subject}
                  onChange={e => setSubject(e.target.value)}
                  className="w-full px-4 py-3 bg-gray-50  border border-gray-200  rounded-xl text-sm text-gray-900  focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 transition-colors duration-200 cursor-pointer"
                >
              ...
              <div>
                <label htmlFor="contact-message" className="block text-xs text-gray-600  mb-2 font-medium">Message</label>
                <textarea
                  id="contact-message"
                  value={message}
                  ...
                  required
                  rows={5}
                  className="w-full px-4 py-3 bg-gray-50  border border-gray-200  rounded-xl text-sm text-gray-900  placeholder-gray-400  focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 transition-colors duration-200 resize-none"
      ...
      <section id="faq" className="py-16 px-6 bg-gray-50 ">
        <div className="max-w-3xl mx-auto">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="space-y-3">
            {faqs.map((faq, i) => (
              <motion.div key={i} variants={fadeUp} className="rounded-xl bg-white  border border-gray-200  overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                <button
                  type="button"
                  onClick={() => setExpandedFaq(expandedFaq === i ? null : i)}
                  aria-expanded={expandedFaq === i}
                  aria-controls={`contact-faq-panel-${i}`}
                  className="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-gray-50  transition-colors cursor-pointer focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-inset"
                >
      ...
      <section className="py-16 px-6">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-8">
          {/* For Lenders */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="rounded-2xl border border-gray-200  bg-white  p-8 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
