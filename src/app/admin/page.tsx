...
function LoginGate({ onAuth }: { onAuth: () => void }) {
  ...
  const login = (e: React.FormEvent) => {
    e.preventDefault();
    if (user === 'admin@clp' && pass === 'admin2026') onAuth();
    else setError('Invalid credentials. Use admin@clp / admin2026');
  };
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm rounded-2xl surface p-8">
        ...
        <p className="text-xs text-slate-500 mb-6">Restricted access</p>
        ...
        <p className="text-[10px] text-slate-600 mt-4 text-center">Demo: admin@clp / admin2026</p>
      </motion.div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex gap-1 mb-8 bg-navy-light/60 rounded-xl p-1 w-fit border border-white/[0.06] overflow-x-auto">
          {([['overview', 'Overview'], ['applications', 'Applications'], ['lenders', 'Lenders'], ['compliance', 'Compliance'], ['revenue', 'Revenue']] as [Tab, string][]).map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)}
              className={`px-5 py-2.5 text-xs rounded-lg transition-colors duration-200 cursor-pointer whitespace-nowrap ${tab === key ? 'bg-navy-light text-slate-50' : 'text-slate-500 hover:text-slate-50'}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        {tab === 'overview' && (
          <div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
              {[
                { label: 'Total Applications', value: String(apps.length), color: 'text-purple-light' },
                { label: 'Funded Loans', value: String(funded), color: 'text-emerald-400' },
                { label: 'Total Revenue', value: `$${totalRevenue.toLocaleString()}`, color: 'text-gold' },
                { label: 'Conversion Rate', value: `${convRate}%`, color: 'text-purple-light' },
              ].map((stat, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                  className="rounded-2xl surface p-6">
                  <div className="text-[10px] text-slate-500 mb-2 uppercase tracking-widest font-medium">{stat.label}</div>
              ...
              <div className="flex items-end gap-3 h-36">
                {monthlyRevenue.map((m, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2">
                    <div className="text-[10px] text-slate-500 font-medium">${(m.revenue / 1000).toFixed(1)}k</div>
                    <motion.div initial={{ height: 0 }} animate={{ height: `${(m.revenue / maxRev) * 100}%` }}
                      transition={{ delay: i * 0.1, duration: 0.6, ease: [0.25, 0.1, 0.25, 1] }}
                      className="w-full bg-gradient-to-t from-purple to-purple-light rounded-t min-h-[4px]" />
                    <div className="text-[9px] text-slate-600 truncate w-full text-center font-medium">{m.month.split(' ')[0]}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {['ID', 'Applicant', 'Vehicle', 'Amount', 'Status', 'Date'].map(h => (
                      <th key={h} className="text-left py-4 px-5 text-[10px] text-slate-500 uppercase tracking-widest font-medium">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {apps.map(app => (
                    <tr key={app.id} className="border-b border-white/[0.04] hover:bg-navy-light/30 transition-colors duration-150">
                      <td className="py-4 px-5 font-mono text-xs text-slate-500">{app.id.slice(0, 10)}</td>
                      <td className="py-4 px-5 font-medium">{app.personalInfo?.firstName} {app.personalInfo?.lastName}</td>
                      <td className="py-4 px-5 text-slate-400">{app.vehicleInfo?.year} {app.vehicleInfo?.make} {app.vehicleInfo?.model}</td>
                      ...
                      <td className="py-4 px-5">
                        <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-medium ${
                          app.status === 'funded' ? 'bg-emerald-500/15 text-emerald-400' :
                          app.status === 'submitted' ? 'bg-purple/15 text-purple-light' :
                          app.status === 'decisioned' ? 'bg-gold/15 text-gold' :
                          'bg-slate-700/50 text-slate-400'
                        }`}>{app.status}</span>
                      </td>
                      <td className="py-4 px-5 text-slate-500 text-xs">{app.createdAt ? new Date(app.createdAt).toLocaleDateString() : ''}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {SAMPLE_LENDERS.map(lender => (
              <div key={lender.id} className="rounded-2xl surface surface-hover p-5 flex flex-col sm:flex-row sm:items-center gap-5">
                ...
                <div className="flex items-center gap-3">
                  <div className={`relative w-11 h-6 rounded-full cursor-pointer transition-colors duration-200 ${lenderActive[lender.id] ? 'bg-purple' : 'bg-slate-700'}`}
                  ...
                  <span className="text-xs text-slate-400 w-14">{lenderActive[lender.id] ? 'Active' : 'Inactive'}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="divide-y divide-white/[0.04]">
              {complianceLogs.map((log, i) => (
                <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  className="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-2 text-sm hover:bg-navy-light/20 transition-colors duration-150">
                  <span className="text-[10px] text-slate-600 font-mono w-36 flex-shrink-0">{log.ts}</span>
                  <span className="font-medium w-48 flex-shrink-0">{log.event}</span>
                  <span className="text-xs text-slate-500 w-32 flex-shrink-0">{log.user}</span>
                  <span className="text-xs text-slate-400 flex-1">{log.detail}</span>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        {tab === 'revenue' && (
          <div className="space-y-6">
                ...
                {SAMPLE_LENDERS.filter(l => l.tier !== 'subprime' || l.id === 'l5').slice(0, 6).map((lender, i) => {
                  ...
                  return (
                    <div key={lender.id} className="flex items-center gap-4">
                      <span className="text-xs text-slate-400 w-44 truncate">{lender.name}</span>
                      <div className="flex-1 h-6 bg-navy-light rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${(rev / 8400) * 100}%` }} transition={{ delay: i * 0.1, duration: 0.6, ease: [0.25, 0.1, 0.25, 1] }}
                          className="h-full bg-gradient-to-r from-gold to-gold-light rounded-full" />
                      </div>
                      <span className="text-xs font-semibold w-16 text-right text-gold">${rev.toLocaleString()}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="rounded-2xl surface overflow-hidden">
              <div className="p-6 border-b border-white/[0.06]"><h3 className="text-sm font-semibold">Monthly Breakdown</h3></div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {['Month', 'Funded Loans', 'Revenue', 'Avg Per Loan'].map(h => (
                      <th key={h} className="text-left py-4 px-6 text-[10px] text-slate-500 uppercase tracking-widest font-medium">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {monthlyRevenue.map((m, i) => (
                    <tr key={i} className="border-b border-white/[0.04]">
                      ...
                      <td className="py-4 px-6 text-slate-400">{m.loans}</td>
                      <td className="py-4 px-6 font-semibold text-emerald-400">${m.revenue.toLocaleString()}</td>
                      <td className="py-4 px-6 text-slate-400">${Math.round(m.revenue / m.loans)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
