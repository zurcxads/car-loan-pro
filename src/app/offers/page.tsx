...
const tierLabels: Record<string, string> = { prime: 'Prime', near_prime: 'Near-Prime', subprime: 'Subprime', specialty: 'Specialty' };
...
export default function OffersPage() {
  ...
  const approved = useMemo(() => offers.filter(o => o.status !== 'declined').sort((a, b) => sort === 'rate' ? a.apr - b.apr : sort === 'payment' ? a.monthlyPayment - b.monthlyPayment : a.termMonths - b.termMonths), [offers, sort]);
  ...
  const confirmOffer = () => {
    ...
    setOffers(updated); saveOffers(updated); setConfirmed(true);
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-md">
          <div className="w-16 h-16 mx-auto mb-8 rounded-full bg-green-500/15 flex items-center justify-center">
            <svg className="w-8 h-8 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
          </div>
          <h2 className="text-2xl font-bold mb-3">Offer Selected</h2>
          <p className="text-zinc-400 text-sm mb-8 leading-relaxed">Your selection has been sent to {selectedOffer?.lenderName}. They will contact you within 1 business day to finalize.</p>
          <Link href="/status" className="inline-flex px-6 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-sm font-medium transition-colors duration-200 cursor-pointer">Track Your Application</Link>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-8">
          ...
          {credit && (
            <div className="flex items-center gap-5 px-6 py-4 rounded-2xl surface">
              <div className="text-center"><div className="text-2xl font-bold text-blue-400">{credit.ficoScore}</div><div className="text-[10px] text-zinc-500 uppercase tracking-widest font-medium mt-0.5">FICO</div></div>
              <div className="w-px h-10 bg-white/10" />
              <div className="text-center"><div className="text-lg font-semibold">{credit.dtiPercent}%</div><div className="text-[10px] text-zinc-500 uppercase tracking-widest font-medium mt-0.5">DTI</div></div>
              <div className="w-px h-10 bg-white/10" />
              <div className="text-center"><div className="text-lg font-semibold">${credit.totalMonthlyObligations.toLocaleString()}</div><div className="text-[10px] text-zinc-500 uppercase tracking-widest font-medium mt-0.5">Monthly</div></div>
            </div>
          )}
        </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-5">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">APR</div><div className="text-xl font-bold text-blue-400">{offer.apr}%</div></div>
                    <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Monthly</div><div className="text-xl font-bold">${offer.monthlyPayment.toLocaleString()}</div></div>
                    <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Term</div><div className="text-xl font-bold">{offer.termMonths} mo</div></div>
                    <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Approved</div><div className="text-xl font-bold">${offer.approvedAmount.toLocaleString()}</div></div>
                  </div>
                  {offer.conditions && offer.conditions.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {offer.conditions.map((c, j) => <span key={j} className="text-[10px] text-zinc-500 px-2.5 py-1 bg-zinc-800 rounded-full">{c}</span>)}
                    </div>
                  )}
                </div>
                <div className="flex flex-col items-end gap-3">
                  <button onClick={() => { setSelectedOffer(offer); setHardPullConsent(false); }}
                    className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-sm font-medium rounded-xl transition-colors duration-200 cursor-pointer whitespace-nowrap">Select This Offer</button>
        ...
        {declined.length > 0 && (
          <div className="mt-10">
            <button onClick={() => setShowDeclined(!showDeclined)} className="text-sm text-zinc-500 hover:text-zinc-50 transition-colors duration-200 cursor-pointer flex items-center gap-2">
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mt-4 space-y-3">
                  {declined.map(offer => (
                    <div key={offer.id} className="surface rounded-xl p-5 opacity-50">
                      <div className="flex items-center justify-between"><span className="text-sm font-medium">{offer.lenderName}</span><span className="text-xs text-red-400 px-2.5 py-0.5 bg-red-500/10 rounded-full border border-red-500/20">Declined</span></div>
        ...
        {selectedOffer && !confirmed && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-6">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} transition={{ duration: 0.2 }}
              ...
              <p className="text-sm text-zinc-400 mb-6">You are selecting the offer from <strong className="text-zinc-50">{selectedOffer.lenderName}</strong> at {selectedOffer.apr}% APR.</p>
              <div className="rounded-xl bg-zinc-800/60 p-5 mb-6 grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-zinc-500 text-xs">APR</span><div className="font-semibold mt-0.5">{selectedOffer.apr}%</div></div>
                <div><span className="text-zinc-500 text-xs">Monthly</span><div className="font-semibold mt-0.5">${selectedOffer.monthlyPayment.toLocaleString()}</div></div>
                <div><span className="text-zinc-500 text-xs">Term</span><div className="font-semibold mt-0.5">{selectedOffer.termMonths} months</div></div>
                <div><span className="text-zinc-500 text-xs">Amount</span><div className="font-semibold mt-0.5">${selectedOffer.approvedAmount.toLocaleString()}</div></div>
              </div>
              <label className="flex items-start gap-3 cursor-pointer mb-6" onClick={(e) => { e.preventDefault(); setHardPullConsent(!hardPullConsent); }}>
                <div className={`mt-0.5 w-5 h-5 rounded-md border flex-shrink-0 flex items-center justify-center transition-colors duration-200 ${hardPullConsent ? 'bg-blue-600 border-blue-600' : 'border-white/20'}`}>
              ...
              <div className="flex gap-3">
                <button onClick={() => setSelectedOffer(null)} className="flex-1 px-4 py-3 text-sm border border-white/10 rounded-xl hover:bg-zinc-800 transition-colors duration-200 cursor-pointer">Cancel</button>
                <button onClick={confirmOffer} disabled={!hardPullConsent}
                  className={`flex-1 px-4 py-3 text-sm font-medium rounded-xl transition-colors duration-200 cursor-pointer ${hardPullConsent ? 'bg-blue-600 hover:bg-blue-500' : 'bg-zinc-700 text-zinc-500 cursor-not-allowed'}`}>Confirm Selection</button>
              </div>
            </motion.div>
          </motion.div>
        )}
