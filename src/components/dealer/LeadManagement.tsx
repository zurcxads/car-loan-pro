...
const statusColors: Record<LeadStatus, string> = {
  ...
  sold: 'border-green-200 bg-green-100 text-green-700',
  lost: 'border-gray-200 bg-gray-100 text-gray-700',
};
...
export default function LeadManagement() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="mb-1 text-lg font-semibold text-gray-900">Lead Management</h2>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-gray-200 bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-600">Lead</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-600">Contact</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-600">Approval</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-600">Vehicle</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-600">Status</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-600">Assigned</th>
                <th className="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-gray-600">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredLeads.map((lead, i) => (
                <tr
                  ...
                  className="animate-fadeIn opacity-0 transition-colors hover:bg-gray-50"
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {lead.status === 'contacted' && (
                        <button
                          ...
                          className="cursor-pointer rounded-lg bg-blue-50 px-3 py-1.5 text-[10px] font-medium text-blue-600 transition-colors hover:bg-blue-100"
                        >
                          Mark Qualified
                        </button>
                      )}
        ...
        {filteredLeads.length === 0 && (
          <div className="py-16 text-center">
            <svg className="mx-auto mb-4 h-10 w-10 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="animate-fadeIn relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-4">
              <div>
                <div className="mb-2 text-sm font-medium text-gray-900">{selectedLead.name}</div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="mb-1 text-[10px] uppercase text-gray-500">Approval Amount</div>
                  <div className="text-lg font-bold text-blue-600">{formatCurrency(selectedLead.approvalAmount)}</div>
                </div>
                <div>
                  <div className="mb-1 text-[10px] uppercase text-gray-500">Credit Tier</div>
                  <div className="text-sm font-medium capitalize text-gray-900">{selectedLead.creditTier}</div>
                </div>
              </div>

              {selectedLead.vehicle && (
                <div>
                  <div className="mb-1 text-[10px] uppercase text-gray-500">Vehicle Interest</div>
                  <div className="text-sm font-medium text-gray-900">{selectedLead.vehicle}</div>
                </div>
              )}

              <div>
                <div className="mb-2 text-[10px] uppercase text-gray-500">Update Status</div>
              ...
              <div>
                <div className="mb-2 text-[10px] uppercase text-gray-500">Notes</div>
                <textarea
            ...
            <div className="mt-6 flex gap-2">
              ...
              <button onClick={() => setSelectedLead(null)} className="cursor-pointer rounded-lg border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50">
                Close
              </button>
            </div>
          </div>
