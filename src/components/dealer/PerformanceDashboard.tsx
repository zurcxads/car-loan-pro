      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x + barW / 2} y={chartH + 18} textAnchor="middle" className="fill-gray-500 text-[11px] ">{String(d[labelKey])}</text>
            <text x={x + barW / 2} y={y - 6} textAnchor="middle" className="fill-gray-700 text-[11px] font-medium ">{val}</text>
          </g>
        );
      })}
    </svg>
...
export default function PerformanceDashboard() {
  ...
  return (
    <div>
      <div className="mb-8 flex w-fit gap-1 rounded-xl border border-gray-200 bg-gray-50 p-1  ">
        {[['month', 'This Month'], ['lastMonth', 'Last Month'], ['90', 'Last 90 Days'], ['ytd', 'YTD']].map(([key, label]) => (
          <button key={key} onClick={() => setRange(key)} className={`cursor-pointer rounded-lg px-4 py-2 text-xs transition-colors ${range === key ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}>{label}</button>
        ))}
      </div>
      ...
      <div className="mb-8 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm  ">
        <h3 className="mb-6 text-sm font-semibold text-gray-900 ">Conversion Funnel</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm  ">
          <h3 className="mb-6 text-sm font-semibold text-gray-900 ">Funded Deals by Week</h3>
          <SimpleBarChart data={weeklyFunded} dataKey="deals" labelKey="week" />
        </div>

        {/* Donut chart */}
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm  ">
          <h3 className="mb-6 text-sm font-semibold text-gray-900 ">Funding by Lender</h3>
