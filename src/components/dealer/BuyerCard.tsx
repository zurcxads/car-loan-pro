...
export default function BuyerCard({ app, invited, onInvite, onViewDetails }: BuyerCardProps) {
  ...
  return (
    <div className={`relative rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md ${invited ? 'border-blue-200' : ''}`}>
      {invited && (
        <div className="absolute right-0 top-0 rounded-bl-xl rounded-tr-2xl border-b border-l border-blue-200 bg-blue-50 px-3 py-1 text-[10px] font-medium text-blue-600">
          Invited
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="ml-auto text-[10px] text-gray-400">Expires in {expiry} days</span>
      </div>

      <div className="mb-3">
        <span className="text-[10px] uppercase tracking-wider text-gray-500">Approved up to</span>
      ...
      <div className="flex gap-6 mb-3 text-sm">
        <div>
          ...
          <div className="font-semibold text-gray-900">{minRate.toFixed(2)}% - {maxRate.toFixed(2)}% APR</div>
        </div>
        <div>
          <span className="text-[10px] text-gray-500">Term</span>
          <div className="font-semibold text-gray-900">{minTerm === maxTerm ? `${minTerm}` : `${minTerm}-${maxTerm}`} months</div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button onClick={onViewDetails} className="cursor-pointer rounded-xl border border-gray-200 px-4 py-2.5 text-xs font-medium transition-colors hover:border-gray-300">
          View Details
        </button>
        {invited ? (
          <button disabled className="flex-1 cursor-not-allowed rounded-xl bg-gray-200 px-4 py-2.5 text-xs font-medium text-gray-500">Invited</button>
        ) : (
