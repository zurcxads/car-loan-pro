...
export default function BuyerCard({ app, invited, onInvite, onViewDetails }: BuyerCardProps) {
  ...
  return (
    <div className={`rounded-2xl surface surface-hover p-5 relative ${invited ? 'border-green-500/20' : ''}`}>
      {invited && (
        <div className="absolute top-0 right-0 bg-green-500/15 text-green-400 text-[10px] font-medium px-3 py-1 rounded-bl-xl rounded-tr-2xl border-b border-l border-green-500/20">
          Invited
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[10px] text-zinc-500">Pre-approved {formatRelativeTime(app.submittedAt)}</span>
        <span className="text-[10px] text-zinc-600 ml-auto">Expires in {expiry} days</span>
      </div>

      <div className="mb-3">
        <span className="text-[10px] text-zinc-500 uppercase tracking-wider">Approved up to</span>
        <div className="text-2xl font-bold text-blue-400">{formatCurrency(maxApproval)}</div>
      </div>

      <div className="flex gap-6 mb-3 text-sm">
        <div>
          <span className="text-[10px] text-zinc-500">Rate Range</span>
          <div className="font-semibold">{minRate.toFixed(2)}% - {maxRate.toFixed(2)}% APR</div>
        </div>
        <div>
          <span className="text-[10px] text-zinc-500">Term</span>
      ...
      <div className="flex gap-2">
        <button onClick={onViewDetails} className="px-4 py-2.5 text-xs border border-white/10 hover:border-white/20 rounded-xl transition-colors cursor-pointer font-medium">
          View Details
        </button>
        {invited ? (
          <button disabled className="flex-1 px-4 py-2.5 text-xs bg-zinc-700 text-zinc-500 rounded-xl cursor-not-allowed font-medium">Invited</button>
        ) : (
          <button onClick={onInvite} className="flex-1 px-4 py-2.5 text-xs bg-blue-600 hover:bg-blue-500 rounded-xl transition-colors cursor-pointer font-medium">
