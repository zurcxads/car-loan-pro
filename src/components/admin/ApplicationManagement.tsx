    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <input
          ...
          className="flex-1 min-w-[250px] px-4 py-2.5 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600"
        />
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-4 py-2.5 text-sm bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-600/20 cursor-pointer">
          <option value="all">All Statuses</option>
          {statuses.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}
        </select>
        <select value={stateFilter} onChange={e => setStateFilter(e.target.value)} className="px-4 py-2.5 text-sm bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-600/20 cursor-pointer">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900">
              <th className="text-left py-3 px-4 text-[10px] text-gray-600 dark:text-zinc-400 uppercase tracking-wider font-medium">ID</th>
          ...
          <tbody>
            {filtered.slice(0, 20).map(app => (
              <>
                <tr
                  key={app.id}
                  className="border-b border-gray-100 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-900 transition-colors cursor-pointer"
                  onClick={() => toggleRow(app.id)}
                >
                  <td className="py-3 px-4 font-mono text-xs text-gray-500 dark:text-zinc-400">{app.id}</td>
                  <td className="py-3 px-4">
                    <div className="text-sm font-medium text-gray-900 dark:text-zinc-100">{app.borrower.firstName} {app.borrower.lastName}</div>
                    <div className="text-xs text-gray-500 dark:text-zinc-400">{app.borrower.email}</div>
                  </td>
                  <td className="py-3 px-4 text-xs text-gray-500 dark:text-zinc-400">{formatRelativeTime(app.submittedAt)}</td>
                  <td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-600 dark:text-zinc-300">{app.lendersSubmitted} sent / {app.offersReceived} offers</td>
                  <td className="py-3 px-4">
                  ...
                  <tr className="bg-gray-50 dark:bg-zinc-900">
                    <td colSpan={8} className="p-6">
                      <div className="grid grid-cols-3 gap-6">
                        <div>
                          <h4 className="text-xs font-semibold text-gray-900 dark:text-zinc-100 mb-3">Credit Profile</h4>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-gray-600 dark:text-zinc-300">FICO:</span>
                              <span className={`font-medium ${ficoColor(app.credit.ficoScore)}`}>{app.credit.ficoScore || 'N/A'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600 dark:text-zinc-300">DTI:</span>
                              <span className={`font-medium ${dtiColor(app.dtiPercent)}`}>{app.dtiPercent}%</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600 dark:text-zinc-300">LTV:</span>
                              <span className={`font-medium ${ltvColor(app.ltvPercent || 0)}`}>{app.ltvPercent || 'N/A'}{app.ltvPercent ? '%' : ''}</span>
                            </div>
                          </div>
                        </div>
                        <div>
                          <h4 className="text-xs font-semibold text-gray-900 dark:text-zinc-100 mb-3">Employment</h4>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-gray-600 dark:text-zinc-300">Income:</span>
                              <span className="font-medium">{formatCurrency(app.employment.grossMonthlyIncome)}/mo</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600 dark:text-zinc-300">Employer:</span>
                              <span className="font-medium text-right">{app.employment.employer}</span>
                            </div>
                          </div>
                        </div>
                        <div>
                          <h4 className="text-xs font-semibold text-gray-900 dark:text-zinc-100 mb-3">Vehicle</h4>
                          <div className="space-y-2 text-sm">
                            {app.vehicle ? (
                              <>
                                <div className="font-medium">{app.vehicle.year} {app.vehicle.make} {app.vehicle.model}</div>
                                <div className="text-gray-600 dark:text-zinc-300">{app.vehicle.mileage?.toLocaleString()} mi</div>
                                <div className="text-gray-600 dark:text-zinc-300">Asking: {formatCurrency(app.vehicle.askingPrice)}</div>
                              </>
                            ) : (
                              <div className="text-gray-500 dark:text-zinc-400">Pre-Approval (No vehicle)</div>
                            )}
                          </div>
                        </div>
                      </div>
